...
import inventoryService, { TypeInventoryItem } from '../../services/inventoryService';

interface ManageTypesModalProps {
  onClose: () => void;
  onTypeAdded: (type: TypeInventoryItem) => void;
}

export default function ManageTypesModal({ onClose, onTypeAdded }: ManageTypesModalProps) {
  ...
  const loadTypes = async () => {
    ...
    try {
      ...
      setError('No se pudieron cargar los tipos. Intente nuevamente.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleAddType = async () => {
    ...
    setError(null);
    try {
      const newType = await inventoryService.createType({ name: newTypeName });
      setTypes([...types, newType]);
      setNewTypeName('');
      onTypeAdded(newType);
    } catch (error) {
      console.error('Error al crear tipo:', error);
      setError('No se pudo crear el tipo. Intente nuevamente.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDeleteType = async (id: number) => {
    ...
    setError(null);
    try {
      await inventoryService.deleteType(id);
      setTypes(types.filter(type => type.id !== id));
    } catch (error) {
      console.error('Error al eliminar tipo:', error);
      setError('No se pudo eliminar el tipo. Intente nuevamente.');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-xl font-medium text-gray-900">Gestionar Tipos de Equipos</h3>
          ...
          <div className="border-t border-gray-100 pt-4">
            <h4 className="text-sm font-medium text-gray-500 mb-3">Tipos Existentes</h4>
            ...
            {isLoading && types.length === 0 ? (
              ...
              <p className="text-center py-4 text-sm text-gray-500">No hay tipos registrados</p>
            ) : (
              <ul className="divide-y divide-gray-100">
                {types.map((type) => (
                  <li key={type.id} className="py-3 flex items-center justify-between">
